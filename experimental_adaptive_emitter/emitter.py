...
class AdaptiveEmitter:
    def __init__(self, entity_id: str, rule_fingerprint: str, code_fingerprint: str) -> None:
        ...
        self.normalizer = RealCausalReplayHarness.__new__(RealCausalReplayHarness)
        self.normalizer.entity_id = entity_id
    ...
    def process(self, physical_row: int, source_row: dict[str, str]) -> dict[str, Any]:
        ...
        observation = self.normalizer.source_row_to_normalized_observation(
            source_row, physical_row - 2
        )
        ...
        capture = self.capturability.evaluate(
            shape, EnvelopeContext.production(evaluation_time=observation.event_time)
        )
        ...
        if self.completed_count < CONTEXT_LENGTH:
            ...
            decision, rule_path = self._decide(
                shape.path_direction.value, vector["H"], vector["C"], adaptive
            )
            ...
            for name, new_value in new_adaptive.items():
                ...
                if old_value != new_value:
                    self.adaptation_audit.append({
                        "property": name, "old_value": old_value, "new_value": new_value,
        ...
        if decision is not None:
            self.feedback_audit.extend([
                {"source_emission_id": emission_id, "feedback_property": "position_decision", "target_state_property": "previous_decision", "old_value": old_decision, "new_value": decision, "equation": "previous_decision_(n+1)=decision_n", "timestamp": source_row["event_timestamp_utc"], "effective_observation": self.completed_count + 2},
                {"source_emission_id": emission_id, "feedback_property": "position_transition", "target_state_property": "position_state", "old_value": old_position, "new_value": next_position, "equation": "BUY->LONG; SELL->SHORT; HOLD->preserve", "timestamp": source_row["event_timestamp_utc"], "effective_observation": self.completed_count + 2},
            ])
