...
**Document:** D01_ADAPTIVE_PARAMETRIC_MODEL_DESIGN_V0_3  
**Version:** 0.3  
...
# Part VA — Fundamental Model Topology

## 33A. Entity-Centric Multi-Input / Multi-Output Model

A fundamental property of D01 is that it is **entity-centric and multi-input / multi-output**.

For each modeled entity \(E_i\), D01 maintains one logical model instance:
...
D01_i
:
N\ \text{Input Channels}
:
M\ \text{Dynamic Model Output Channels}
}
\]

This is a cardinality statement about the logical model.

It does **not** imply a one-to-one relationship between an individual input channel and an individual output channel.
...
is the set of input channels available to the model instance for entity \(E_i\), and:
...
is the fan-out set of Dynamic Model Output channels associated with the same modeled entity.
...
N INPUT CHANNELS
        |
        v
ONE LOGICAL D01 MODEL INSTANCE
        |
        v
M DYNAMIC MODEL OUTPUT CHANNELS
```

For the initial SPY experiment:
...
ENTITY: SPY
    ...
    +-- ask size
    +-- spread
    +-- volatility-derived state
    +-- event / perturbation context
    +-- other permitted contextual channels
             |
             v
          D01-SPY
             |
    ...
    |        |         |          |
    v        v         v          v
direction  magnitude  persistence half-life
                                   |
                          +--------+-------- ...
                          |
                          v
...
## 33B. Input Channels Belong to Model Context, Not Necessarily to the Entity

The modeled entity and the origin of every input channel need not be identical.

D01 may consume two broad input families.

### Intrinsic channels

Observations directly associated with the modeled entity.

For SPY, examples include:
...
- SPY spread.
- SPY quote size.
- SPY-derived velocity and acceleration.
- SPY-local perturbation state.

### Contextual / cross-entity channels

Observations originating outside the modeled entity but potentially relevant to its state.

Future examples could include:

- volatility-index state,
- index futures,
- Treasury movement,
- broad market state,
...
related instrument -----------+----> D01-SPY ----> SPY DMO CHANNELS
event / perturbation ---------+
                              |
other permitted context ------+
```

All contextual channels must obey the same point-in-time causality rule as intrinsic channels.

The resulting DMO remains explicitly associated with the modeled entity \(E_i\).

## 33C. DMO Fan-Out Is Not Decision Fan-Out

The output channels from D01 are **Dynamic Model Outputs**, not trading decisions.

D01 may emit channels representing:
...
- decay,
- reversal tendency/risk,
- and Forward Model Outputs over one or more temporal regions.

These are model inferences.

They must not be named or treated as BUY/SELL/TRADE decisions.

The downstream distinction remains:

```text
D01  model inference / DMO
 |
 v
D02  Return Shape
 |
 v
D04  Capturability / Trading Envelope
 |
 v
D05  Opportunity
 |
...
## 33D. Structured Multi-Output Mapping

The general D01 model is therefore better represented as:
...
- \(\mathbf X_i(t)\) = intrinsic and contextual input-channel state.
- \(\boldsymbol\theta_i(t)\) = adaptive parameter state.
- \(\mathbf H_i(t)\) = adaptive temporal / half-life state.
- \(\mathbf S_i(t)\) = Adaptive Signal state.
- \(\mathbf P_i(t)\) = perturbation/event state.
- \(\mathbf Y_i(t)\) = structured DMO fan-out for entity \(E_i\).

Not every output must be a scalar.

Therefore \(\mathbf Y_i(t)\) is logically a structured output space rather than necessarily a simple numeric vector.

## 33E. Polynomial Expansion Is an Internal Candidate Mechanism

The bounded \(n\)-th-order polynomial discussed elsewhere in this design is **not the definition of D01**.

The fundamental definition is:

\[
\boxed{\mathcal M_i:\mathbf X_i\rightarrow\mathbf Y_i}
\]

A polynomial/basis expansion is one candidate mechanism inside that mapping.

For output channel \(j\), an initial candidate family is:
...
- \(j\) identifies a DMO output channel.
- \(k,l\) identify input channels.
- \(q\) identifies polynomial/basis order.
- \(n_{jk}\) is the bounded structural order permitted for input \(k\) when contributing to output \(j\).
- \(x_{ik}^{*}(t)\) is the temporally transformed input.
- \(\theta_{jkq}(t)\) are adaptive coefficients.
- \(\beta_{jkl}(t)\) are candidate interaction coefficients.

This permits the same input channel to influence different DMO channels differently.

For example, volume may have one learned relationship with:
...
There is no requirement that volume have one universal model meaning.

## 33F. Three Adaptive Dimensions

The entity-centric MIMO topology clarifies three distinct forms of adaptation:

\[
\boxed{
\begin{aligned}
\boldsymbol\theta(t) &:\quad \text{parameter adaptation}\\
\mathbf H(t) &:\quad \text{temporal adaptation}\\
\mathbf n &:\quad \text{structural adaptation}
\end{aligned}
}
\]

Parameter and temporal adaptation may operate relatively quickly.

Structural adaptation—such as changing polynomial/basis order or interaction topology—should initially occur more slowly and only through controlled validation.

A perturbation may therefore change:
...
without immediately changing model structure.

## 33G. Entity Isolation and Shared Context

Each modeled entity should retain independently auditable:
...
Shared contextual observations may feed multiple entity models, but this must not collapse their model states into one undifferentiated global state.
...
This allows common market perturbations to influence multiple entities while preserving entity-specific responses.

## 33H. D01 to D02 Cardinality

For each modeled entity:
...
where \(\mathcal R_i(t)\) is the entity's elastic Return Shape.

D02 therefore consumes the relevant DMO fan-out rather than one scalar forecast.
...
M DMO CHANNELS
     ...
     D02_i
       |
       v
ONE ELASTIC RETURN-SHAPE REPRESENTATION
for modeled entity E_i
```

The Return Shape may itself contain multiple temporal regions or candidate forward forms.

## 33I. Fundamental Model Property — Design Commitment

The following is now a D01 design commitment:

> **D01 is an entity-centric, adaptive, multi-input / multi-output model. One logical D01 model instance represents one modeled entity, consumes many intrinsic and contextual input channels, and emits a structured fan-out of Dynamic Model Output channels for that entity. Individual inputs are not mapped one-to-one to individual outputs; their relationships are adaptive, potentially nonlinear, temporally weighted, and interaction-dependent.**

And:

> **The mathematical model family—including bounded nth-order polynomial/basis expansions—is subordinate to this topology. The topology defines D01; the polynomial family is one experimentally replaceable implementation of the mapping.**
...
- D01 is entity-centric: one logical model instance represents one modeled entity.
- D01 is multi-input / multi-output (MIMO): N input channels may produce M structured DMO channels.
- Input/output relationships are not one-to-one and may be nonlinear, temporal, and interaction-dependent.
- Contextual cross-entity observations may feed an entity model while outputs remain entity-specific.
