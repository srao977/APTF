# Adaptive Parametric Trading Framework — Reference 1.1
...
| ID  | Design Artifact | Status |
|-----|-----------------|--------|
| D01 | Adaptive Parametric Model | CONCEPT |
| D02 | Return Shape / Forward State Model | CONCEPT |
| D03 | Real-Time Decision System | CONCEPT |
| D04 | Trading Envelope | EXPERIMENTAL |
| D05 | Opportunity Engine | CONCEPT |
| D06 | Decision Engine | CONCEPT |
| D07 | Independent Risk Engine | CONCEPT |
| D08 | Position / Trade Lifecycle Manager | CONCEPT |
| D09 | Execution Policy Engine | CONCEPT |
| D10 | Broker Adapter | CONCEPT |
| D11 | Internal Replay / Paper Broker | CONCEPT |
| D12 | Alpaca Paper Integration | CONCEPT |
| D13 | Outcome / Feedback System | CONCEPT |
| D14 | Model Registry / Promotion Controller | CONCEPT |
| D15 | Capital / Portfolio Allocation | CONCEPT |
| D16 | Market Universe / Candidate Scanner | CONCEPT |
| D17 | Market Clock / Event Context | CONCEPT |
| D18 | Data Quality / Time Synchronization | CONCEPT |
| D19 | Observability / Control / Recovery | CONCEPT |
| D20 | Overall System Logical Design | CONCEPT |

### D01 — Adaptive Parametric Model

Purpose:

The learning and intelligence core of APTF.

It consumes observations and learns/adapts parameter relationships,
state representations, state transitions, nonlinear behavior, interactions,
regimes, and forward-relevant structure.
...
D01 supplies validated intelligence to downstream real-time components.

### D02 — Return Shape / Forward State Model

Purpose:

Convert D01 intelligence plus current observations into the elastic forward
Return Shape consumed by the real-time Trading Envelope.
...
D02
  ->
Return Shape R_i(t,h)
```

D02 should preserve uncertainty, horizon, direction, magnitude, persistence,
decay, reversal risk, and other forward-state characteristics.

D02 mathematics is intentionally not finalized in this design update.

### D03 — Real-Time Decision System

Purpose:

The real-time integration boundary that orchestrates:

- live observations
- trigger/state updates
- D01/D02 inference outputs
...
It is not itself the Adaptive Learning Model.

It is the real-time operating system that consumes validated intelligence.

### D04 — Trading Envelope

Trading Envelope remains D04.

The active implementation and physical folder naming remain unchanged.

### Dependency View (Design Orientation)

``` text
D01 Adaptive Parametric Model
            |
            v
D02 Return Shape / Forward State
            |
            v
D03 Real-Time Decision System
            |
...
Supporting/side-state artifacts:

- D08 Position Lifecycle
- D11 Internal Replay / Paper Broker
- D12 Alpaca Paper
- D13 Outcome / Feedback
- D14 Model Registry / Promotion
- D15 Capital / Portfolio Allocation
- D16 Market Universe / Scanner
- D17 Market Clock / Events
- D18 Data Quality / Time
- D19 Observability / Control / Recovery
- D20 Overall System Logical Design

This dependency view is a design-orientation map and not a strict runtime
pipeline for every artifact.

> D01 is the intelligence core. D02 expresses its forward inference as an
> elastic Return Shape. D04 determines whether that Return Shape is
> capturable within the current Trading Envelope. Downstream artifacts
> convert qualified opportunities into controlled execution.

### Next Major Design Artifact After D04 Experimental Prototype V0.2
...
``` text
D04 synthetic validation
      ->
D01 logical design
      ->
D01 physical experimental model
      ->
D02 Return Shape generation
      ->
historical SPY replay through D01/D02/D04
      ->
real-time SPY
      ->
Alpaca Paper
...
This sequence may evolve as experiments dictate.
...
**Reference 1.1 note:** This is the original simplified architectural view. The later D01–D20 artifact map and consolidated Return Shape → Trading Envelope → Opportunity architecture are authoritative for current design ordering.
...
### Provider-Neutral Market Data and EODHD Candidate Source

The Market Data Gateway is deliberately provider-neutral. D01 must never know which vendor generated an observation. Provider-specific schemas, authentication, transport, reconnect behavior, and timestamp conventions terminate at the gateway / D18 data-quality boundary.

Current candidate sources include:
...
EODHD is currently positioned as a **market / external-data source**, not as a replacement for the broker/execution destination:
...
For APTF, integration should use the underlying source interfaces directly: REST for historical, reference, and event datasets, and WebSocket transport where genuinely real-time observations are required. The EODHD MCP layer is not part of the production data plane. MCP may remain useful outside APTF as a human/LLM research interface, but the real-time system should use deterministic, typed machine-to-machine data ingestion.

EODHD-specific code should therefore live behind a small provider adapter rather than inside D01. The same normalized observation contract must be usable by EODHD, Alpaca market data, another future provider, and historical replay.

### Initial EODHD Experimental Progression

The initial EODHD use is an engineering/model experiment, not evidence that EODHD has already been validated as the production real-time feed. The progression is:
...
D04 Trading Envelope
      ↓
Capturability / subsequent observed outcome
      ↓
Evaluate predictive value of Forward Model Outputs
```

Free access should be used first to prove the adapter, normalization path, D01/D02 behavior, and point-in-time evaluation method. If the experiment demonstrates useful behavior, a paid EODHD tier can then be considered for SPY and richer/live data. Current plan pricing and exact SPY/real-time entitlements must be re-verified at subscription time rather than treated as a permanent architectural assumption.
...
-   Forward-output horizons
-   Forward-output thresholds
...
D01 does not primarily produce a static "forecast." It continuously emits **Dynamic Model Outputs (DMO)** as its parameters, inferred state, uncertainty, relationships, and forward state evolve with new observations.

**Dynamic Model Output (DMO)** — any state, parameter, relationship, transition, uncertainty, or forward inference emitted by D01 at a particular model time.

**Forward Model Output (FMO)** — the subset of Dynamic Model Outputs describing possible future evolution over one or more horizons.

**Forecast** — a testable forward assertion extracted from an FMO and later evaluated against an observed outcome. Forecast is therefore retained as an experimental/evaluation term, not as the general name for D01 output.
...
The principal interface is therefore:

```text
D01 DMO → D02 Return Shape
```

D02 does not receive one frozen forecast. It receives a changing collection of model outputs and continuously expresses the relevant forward information as an elastic Return Shape.

A Dynamic Model Output is valid **at a model time**. It is not permanently valid merely because its forward horizon has not yet elapsed. As new observations arrive, the DMO/FMO may strengthen, weaken, reverse, change uncertainty, or expire. A five-minute FMO generated at one instant may therefore differ materially from the five-minute FMO generated seconds later because the underlying inferred state has evolved.

The model loop is consequently:
...
new observations
    ↓
revised Dynamic Model Outputs
    ↓
... continuous evolution ...
```

This terminology prevents APTF from being reduced conceptually to a static forecasting system. Its broader purpose is continuous estimation and expression of an evolving system state, including forward-relevant structure.
...
The resulting Reference 1.1 production architecture is:
...
## 36. Reference 1.1 Architectural Principle
...
## 45. Dynamic Model Output Evaluation and Feedback

Forward Model Outputs create testable forward assertions that can be evaluated later and returned to the Adaptive Learning System. The general D01 output remains dynamic; only a specifically captured assertion used for point-in-time evaluation is treated as a **forecast**.

For an FMO emitted at model time `t`, store both the immutable evaluation snapshot and the continuing DMO stream:

```text
DYNAMIC MODEL OUTPUT AT t
      |
      +----> continuing DMO/FMO revisions as state evolves
      |
      v
CAPTURED TESTABLE FMO ASSERTION AT t
      |
      v
OBSERVED STATE / OUTCOME AT t+h
      |
      v
FMO EVALUATION ERROR / OUTCOME
      |
      v
ADAPTIVE LEARNING SYSTEM
```

This distinction allows the system to measure predictive value without pretending that the live model output should remain frozen until `t+h`. Feedback should include both trading outcomes and FMO/state-evolution outcomes.
...
The authoritative Reference 1.1 real-time spine is:
...
Current State / Regime
        ↓
D01 Validated Adaptive Inference
        ↓
D02 Forward State / Return Shape
        ↓
Return Field F(t)
        ↓
D04 Trading Envelope
        ├── Base Capturability B_i(t)
        ├── Feasibility Gate G_i(t)
        └── C_i(t) = B_i(t) × G_i(t)
        ↓
Adaptive Aperture / Hysteresis
        ↓
D05 Opportunity Engine
        ↓
D06 Decision Engine
        ↓
D07 Independent Risk Engine
        ↓
D09 Execution Policy Engine
        ↓
D10 Broker Adapter
```
...
**Reference 1.1 ordering clarification:** D02 produces the Return Shape; D04 evaluates that shape against the Trading Envelope and its Capturability/aperture conditions; D05 then operates on shapes that have qualified through D04 to construct, rank, or further evaluate actionable opportunities. Older candidate-opportunity-first wording below is retained as historical design context and is not the authoritative D01–D10 dependency order.
...
### D04 Experimental Capturability Semantics — V0.2

The current D04 experimental physical prototype separates base Capturability from non-compensating feasibility:

\[
\boxed{C_i(t)=B_i(t)\times G_i(t)}
\]

where:

- \(B_i(t)\) is the experimental base Capturability derived from Return Shape and soft envelope quality, including opportunity lifetime.
- \(G_i(t)\) is the experimental feasibility gate representing conditions that can make an otherwise attractive Return Shape physically or operationally uncapturable.
- The V0.2 prototype currently uses the minimum configured gate dimension as \(G_i(t)\). This is deliberately conservative and remains experimental rather than final production mathematics.

> **A strong Return Shape cannot compensate for a Trading Envelope condition that makes the shape physically or operationally uncapturable.**

> **Capturability is not merely a weighted attractiveness score. It represents the realizability of a Return Shape under the active Trading Envelope.**
